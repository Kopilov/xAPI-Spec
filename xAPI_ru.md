...
данными между LRS им дана некоторая гибость на поддержиавемых версиях Утверждений.
...
версия указана не была, заданное значение ОБЯЗЯНО быть 1.0.0.
...
* Согласно RFC 1341, разделителем является строка boundary, указанная в заголовке, окружённая последовательностями ```<CRLF>```.
